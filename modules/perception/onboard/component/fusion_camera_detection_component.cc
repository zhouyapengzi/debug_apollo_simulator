...
AINFO<<"(DMCZP) EnteringMethod: fill_lane_msg";
  ...
  AINFO<<"(DMCZP) LeaveMethod: fill_lane_msg";
 }
...
AINFO<<"(DMCZP) EnteringMethod: FusionCameraDetectionComponent::InitConfig";
  ...
  camera_perception_init_options_.lane_calibration_working_sensor_name =
    ...
    AINFO<<"(pengzi)load camera obstacle perception conf_file: "
        ...
        << " calibration working sensor name: " << camera_perception_init_options_.lane_calibration_working_sensor_name ;
...
AINFO<<"(DMCZP) EnteringMethod: FusionCameraDetectionComponent::OnMotionService";
  ...
  AINFO<<"(DMCZP) LeaveMethod: FusionCameraDetectionComponent::OnMotionService";
 }
...
AINFO<<"(DMCZP) EnteringMethod: FusionCameraDetectionComponent::SetCameraHeightAndPitch";
  ...
  AINFO<<"(DMCZP) LeaveMethod: FusionCameraDetectionComponent::SetCameraHeightAndPitch";
 }
...
AINFO<<"(DMCZP) EnteringMethod: FusionCameraDetectionComponent::InternalProc";
  ...
  AINFO << "(pengzi)##" << camera_name << ": pitch "
        << camera_frame.calibration_service->QueryPitchAngle()
        << " | camera_grond_height "
        << camera_frame.calibration_service->QueryCameraToGroundHeight()<< " thread: "<<std::this_thread::get_id();;
